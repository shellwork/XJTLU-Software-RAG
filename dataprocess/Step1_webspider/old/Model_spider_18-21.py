...
model_folder = './data/wiki_text_2018-2021'  # Set the output path for text files
...
def fetch_urls_from_csv(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as csv_file:
        ...
        for row in reader:
            year = row['year']
            team_name = row['team']
            base_url = f"https://{year}.igem.org/Team:{team_name}/Model"
            data.append({'year': year, 'team_name': team_name, 'url': base_url})
    return data
