...
text_folder = './data/wiki_texts'  # Set the output path for text files
...
def fetch_urls_from_csv(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as csv_file:
        ...
        for row in reader:
            year = row['year']
            team_name = row['team']
            url = f"{row['wiki']}/parts" if not row['wiki'].endswith('/parts') else row['wiki']
    ...
    for entry in data:
        ...
        for attempt in range(max_retries):
            try:
                ...
                main_soup = BeautifulSoup(main_response.content, 'lxml', from_encoding='utf-8')
